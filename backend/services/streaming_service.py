...
async def _process_block_async(
    ...
    try:
        ...
        # CRITICAL: Validate part is a dict before storing
        if not isinstance(part, dict):
            logger.error(f"[WORKFLOW] Invalid part type for {block_name}: {type(part)}, skipping")
            return
        ...
        # CRITICAL: Validate part_object has numeric price/quantity
        if isinstance(part_object.get("price"), dict):
            logger.warning(f"[WORKFLOW] Part {block_name} has dict price, normalizing")
            from utils.cost_utils import safe_extract_cost
            part_object["price"] = safe_extract_cost(part_object["price"], default=0.0)
        if not isinstance(part_object.get("quantity"), (int, float)):
            part_object["quantity"] = 1
...
async def generate_design_stream(query: str, orchestrator: StreamingOrchestrator, queue: asyncio.Queue):
    ...
    try:
        ...
        # Parallel datasheet enrichment with timeout handling
        ...
        if selected_parts:
                ...
                if part_id:
                    task = asyncio.create_task(
                        asyncio.wait_for(
                            ...
                            timeout=10.0  # 10 second timeout per part enrichment
                        )
                    )
            ...
            for block_name, task in enrichment_tasks:
                try:
                    ...
                    logger.warning(f"Datasheet enrichment timeout for {block_name} after 10s")
                    # Keep original part data on timeout
                except Exception as e:
                    logger.warning(f"Datasheet enrichment error for {block_name}: {e}")
        ...
        # Parallel output generation with timeout handling
        connections_task = asyncio.create_task(
            asyncio.wait_for(
                ...
                timeout=20.0  # 20 second timeout for connection generation
            )
        )
        ...
        bom_task = asyncio.create_task(
            asyncio.wait_for(
                ...
                timeout=20.0  # 20 second timeout for BOM generation
            )
        )
        ...
        try:
            ...
            logger.error("[WORKFLOW] Output generation timed out after 20s")
            await queue.put({
                "type": "error",
                "message": "Output generation timed out. Using minimal outputs..."
            })
            # Create minimal outputs to prevent errors
            connections = []
            bom = {
                "items": [],
                "summary": {"total_cost": 0.0, "total_parts": 0, "total_qty": 0},
                "metadata": {}
            }
        except Exception as e:
            logger.error(f"[WORKFLOW] Output generation error: {str(e)}")
            await queue.put({
                "type": "error",
                "message": f"Output generation error: {str(e)}. Using minimal outputs..."
            })
            # Create minimal outputs to prevent errors
            connections = []
            bom = {
    ...
    try:
        ...
        try:
            connections = await asyncio.wait_for(
                asyncio.to_thread(
                    output_gen.generate_connections,
                    orchestrator.design_state["selected_parts"],
            ...
            logger.warning(f"[REFINEMENT] Connection generation error: {e}")
            orchestrator.design_state["connections"] = []
        
        try:
            bom = await asyncio.wait_for(
                asyncio.to_thread(
                    output_gen.generate_bom,
                    orchestrator.design_state["selected_parts"],
            ...
            logger.warning(f"[REFINEMENT] BOM generation error: {e}")
            orchestrator.design_state["bom"] = {
                "items": [],
                "summary": {"total_cost": 0.0, "total_parts": 0, "total_qty": 0},
                "metadata": {}
            }
...
async def answer_question(
    ...
    try:
        ...
        if question_type == "cost":
            ...
            try:
                cost_analysis = await asyncio.wait_for(
                    asyncio.to_thread(cost_agent.analyze_bom_cost, bom_items),
                    timeout=15.0  # 15 second timeout for cost analysis
                )
                ...
                logger.error("[QUESTION] Cost analysis timed out after 15s")
                answer = "Cost analysis timed out. Please try again."
            except Exception as e:
            ...
            for block_name, part in orchestrator.design_state["selected_parts"].items():
                ...
                if isinstance(part, dict):
                    ...
                    logger.error(f"[BOM_VALIDATION] Invalid part type for {block_name}: {type(part)}")
            try:
                power_analysis = await asyncio.wait_for(
                    asyncio.to_thread(power_agent.calculate_power_consumption, bom_items, {}),
                    timeout=15.0  # 15 second timeout for power analysis
                )
                total_power = power_analysis.get("total_power", 0)
                ...
                logger.error("[QUESTION] Power analysis timed out after 15s")
                answer = "Power analysis timed out. Please try again."
            except Exception as e:
