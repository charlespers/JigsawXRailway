...
def part_data_to_part_object(part_data: Dict[str, Any], quantity: int = 1, component_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    # CRITICAL: Ensure quantity is always a positive integer
    if not isinstance(quantity, (int, float)) or quantity < 1:
        quantity = 1
    quantity = int(quantity)
    
    # CRITICAL: Ensure unit_cost is always a float
    if not isinstance(unit_cost, (int, float)):
        unit_cost = 0.0
    unit_cost = float(unit_cost)
    ...
    part_object = {
        ...
        "price": unit_cost,  # Already validated as float above
        "currency": "USD",
        "voltage": voltage,
        "package": part_data.get("package", ""),
        "interfaces": interfaces,
        "datasheet": part_data.get("datasheet_url", ""),
        "quantity": quantity,  # Already validated as int above
        # Extended fields
