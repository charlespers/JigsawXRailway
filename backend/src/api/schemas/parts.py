...
class BOMEDAAssetsRequest(BaseModel):
    """BOM EDA assets request"""
    bom_items: List[Dict[str, Any]] = Field(..., min_items=1)
    tool: str = Field(default="kicad", pattern="^(kicad|altium|eagle)$")
    asset_types: List[str] = Field(default_factory=lambda: ["footprint", "symbol"])


class BOMEDAAssetsResponse(BaseModel):
    """BOM EDA assets response"""
    assets: Dict[str, Any]
    tool: str
    part_count: int
