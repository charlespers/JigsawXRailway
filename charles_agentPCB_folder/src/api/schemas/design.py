...
class DesignReuseRequest(BaseModel):
    """Request for design reusability analysis"""
    bom_items: List[BOMItem]
    existing_designs: Optional[List[DesignState]] = Field(default_factory=list)


class DesignReuseResponse(BaseModel):
    """Design reusability analysis response"""
    reusable_blocks: List[Dict[str, Any]]
    component_libraries: Dict[str, Any]
    similarity_analysis: Optional[Dict[str, Any]] = None
    standardization_score: float = Field(..., ge=0, le=100)
