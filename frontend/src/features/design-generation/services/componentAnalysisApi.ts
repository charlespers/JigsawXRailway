...
async function realStartAnalysis(
  ...
  try {
    ...
    const STREAM_TIMEOUT_MS = 300000; // 5 minutes max for entire stream
    const HEARTBEAT_TIMEOUT_MS = 60000; // 1 minute max between events

    while (true) {
      // Check for total stream timeout
      const totalElapsed = Date.now() - streamStartTime;
      if (totalElapsed > STREAM_TIMEOUT_MS) {
        throw new Error("Stream timeout: Analysis took longer than 5 minutes");
      }

      // Check for heartbeat timeout (no events for too long)
      const timeSinceLastEvent = Date.now() - lastEventTime;
      if (timeSinceLastEvent > HEARTBEAT_TIMEOUT_MS) {
        throw new Error("Stream heartbeat timeout: No events received for 1 minute");
      }
      ...
      while ((eventEndIndex = buffer.indexOf("\n\n")) !== -1) {
        ...
        if (dataLine) {
          try {
            ...
            // Update heartbeat timer on any event received (not just on read)
        ...
        .find((line) => line.startsWith("data: "));
          if (dataLine) {
            try {
              ...
              if (jsonText) {
                const data: ComponentAnalysisResponse = JSON.parse(jsonText);
                lastEventTime = Date.now(); // Update heartbeat on final event
                onUpdate(data);
              }
            } catch (e) {
