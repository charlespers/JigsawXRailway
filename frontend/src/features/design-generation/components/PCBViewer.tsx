...
// Color palette for different component types (matches demo's component IDs)
const getComponentColor = (componentId: string): string => {
  const idLower = componentId.toLowerCase();
  const colorMap: Record<string, string> = {
    // Main components
    anchor: "#10b981", // MCU/anchor component
    mcu: "#10b981",
    power: "#3b82f6",
    sensor: "#f59e0b",
    sensors: "#f59e0b",
    ...
    connector: "#14b8a6",
    // New component types
    intermediary: "#f97316", // Orange for voltage converters/level shifters
    test_point: "#06b6d4", // Cyan for test points
    fiducial: "#a855f7", // Purple for fiducials
    // Architecture blocks
    requirements: "#64748b",
    architecture: "#64748b",
    analysis: "#64748b",
  ...
  // Check for partial matches (e.g., "intermediary_power", "test_point_3v3")
  if (idLower.includes("intermediary")) return colorMap.intermediary;
  if (idLower.includes("test_point") || idLower.startsWith("tp")) return colorMap.test_point;
  if (idLower.includes("fiducial") || idLower.startsWith("fid")) return colorMap.fiducial;
  
  return colorMap[idLower] || "#10b981";
};
...
const getComponentSize = (componentId: string, _partData?: PartObject): { w: number; h: number } => {
  ...
  // Special handling for test points and fiducials (smaller)
  if (idLower.includes("test_point") || idLower.startsWith("tp")) {
    return { w: 8 * baseScale, h: 8 * baseScale };
  }
  if (idLower.includes("fiducial") || idLower.startsWith("fid")) {
    return { w: 6 * baseScale, h: 6 * baseScale };
  }
  
  const sizeMap: Record<string, { w: number; h: number }> = {
    anchor: { w: 60 * baseScale, h: 60 * baseScale },
    ...
    intermediary: { w: 35 * baseScale, h: 25 * baseScale },
  };
  
  // Check for partial matches
  if (idLower.includes("intermediary")) return sizeMap.intermediary;
  
  return sizeMap[idLower] || { w: 40 * baseScale, h: 40 * baseScale };
};
...
function determineConnections(components: PCBComponent[]): Connection[] {
  ...
  // Support both old naming (mcu) and new naming (anchor)
  const anchor = components.find((c) => {
    const id = c.id.toLowerCase();
    return id === "anchor" || id === "mcu";
  });
  const mcu = anchor; // Alias for compatibility
  const power = components.find((c) => {
    const id = c.id.toLowerCase();
    return id === "power" || id.includes("power");
  });
  const connector = components.find((c) => {
    const id = c.id.toLowerCase();
    return id === "connector" || id.includes("connector");
  });
  const sensors = components.find((c) => {
    const id = c.id.toLowerCase();
    return id === "sensor" || id === "sensors" || id.includes("sensor");
  });
  const memory = components.find((c) => {
    const id = c.id.toLowerCase();
    return id === "memory" || id.includes("memory");
  });
  const antenna = components.find((c) => {
    const id = c.id.toLowerCase();
    return id === "antenna" || id.includes("antenna");
  });
  const passives = components.find((c) => {
    const id = c.id.toLowerCase();
  ...
  if (!anchor && !mcu) return connections;
...
export default function PCBViewer({ selectedComponents = new Map() }: PCBViewerProps) {
  ...
  useEffect(() => {
    ...
    componentsArray.forEach((comp) => {
      ...
      const size = comp.size || getComponentSize(comp.id, comp.partData);
      const x = comp.position.x;
      const y = comp.position.y;
      const idLower = comp.id.toLowerCase();
      const isTestPoint = idLower.includes("test_point") || idLower.startsWith("tp");
      const isFiducial = idLower.includes("fiducial") || idLower.startsWith("fid");

      // Special rendering for test points and fiducials
      if (isTestPoint) {
        // Draw test point as a small circle
        ctx.fillStyle = color;
        ctx.strokeStyle = color;
        ctx.lineWidth = 1 / zoom;
        ctx.beginPath();
        ctx.arc(x + size.w / 2, y + size.h / 2, size.w / 2, 0, Math.PI * 2);
        ctx.fill();
        ctx.stroke();
      ...
      if (isFiducial) {
        // Draw fiducial as a small filled circle with crosshair
        ctx.fillStyle = color;
        ctx.strokeStyle = color;
        ctx.lineWidth = 1 / zoom;
        ctx.beginPath();
        ...
        ctx.beginPath();
        ctx.moveTo(x + size.w / 2 - size.w / 3, y + size.h / 2);
        ctx.lineTo(x + size.w / 2 + size.w / 3, y + size.h / 2);
        ctx.moveTo(x + size.w / 2, y + size.h / 2 - size.h / 3);
        ctx.lineTo(x + size.w / 2, y + size.h / 2 + size.h / 3);
        ctx.stroke();
        return;
      }

      // Draw regular component (green for selected)
      ...
      // Draw pins (smaller, more realistic) - skip for very small components
      ...
      const label = comp.label.length > 12 ? comp.label.substring(0, 10) + ".." : comp.label;
      ctx.fillText(label, x + size.w / 2, y + size.h / 2);
