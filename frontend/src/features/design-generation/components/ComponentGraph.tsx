...
interface ComponentGraphProps {
  ...
  provider?: "xai"; // AI provider to use for analysis
  isAnalyzing?: boolean;
  ...
  onContextRequested?: (queryId: string, message: string) => void; // Callback when design system requests context during analysis
...
export default function ComponentGraph({
  ...
  useEffect(() => {
    ...
    // Ensure provider is valid, default to "xai" if not
    const validProvider = provider === "xai" ? provider : "xai";
    componentAnalysisApi
      .startAnalysis(
        analysisQuery,
        validProvider,
      ...
      .catch((error: any) => {
        ...
        if (!isCancellation) {
          ...
          const errorMessage = isTimeout 
            ? "Analysis timed out. The workflow may have taken too long. Please try again with a simpler query."
            : (error.message || "Analysis failed. Please try again.");
          setError(errorMessage);
          
          // If timeout, emit a complete event to prevent hanging
          if (isTimeout && onAnalysisComplete) {
            onAnalysisComplete();
          }
